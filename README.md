...
| **再生失敗の検知** | 即時（90秒のデバウンス付き） | `log stream` で `coreaudiod` を購読し、失敗シグネチャを見つけたら**失敗したデバイスを特定してから**復旧する |

複数回に分けているのは、復帰直後はデバイスがまだ戻りきっていないことがあるためです。オーディオデバイスは毎回 UID で引き直します（`AudioObjectID` は起動やスリープのたびに変わります）。

対象は**トランスポート種別が HDMI / DisplayPort の出力デバイスすべて**です。機種も UID も決め打ちしていないので、モニターを入れ替えてもそのまま動きます。ただし**ログから失敗を検知した場合だけは、失敗したデバイス1台に絞ります**（後述）。AirPods や内蔵スピーカーの再生失敗で、無関係な HDMI デバイスのサンプルレートを触らないためです。

3つの契機はどれも同じ復旧処理を呼ぶので、**すべて単一のシリアルキューと `flock` で直列化**しています。並行して走ると、片方が退避した元レートをもう片方が「現在値」として読み、48kHz のデバイスを 44.1kHz で確定させてしまいます。`flock` は常駐プロセスと手動の `audio-fix repair` がぶつかる場合のためのものです。

最後に**既定出力を一瞬別デバイスへ切り替えて戻します**（いま直したデバイスが既定出力になっている場合のみ）。Slack や Discord などの Electron（Chromium）アプリは出力デバイスをキャッシュするため、デバイスが作り直されると古いハンドルを掴んだまま無音になることがあります（**アプリを再起動すると直り、ログには再生記録すら残らない**のが特徴）。デバイス変更通知を飛ばして再取得させるのが狙いです。
...
- **失敗行そのものにはデバイスが載りません。** UID もエラー詳細も `<private>` でマスクされます。手がかりはコンテキストIDだけで、対応する UID は同じ事象の約10秒前に出る `IOWorkLoopInit` 行にあります。この対応表を持っておくと、どのデバイスの失敗かを特定できます
  ...
  `StartIOThread: the IO thread failed to start` もこの不具合で必ず出ますが、コンテキストIDを持たず対象を特定できません。実測では上の行と対で出るため、復旧の契機には使っていません
- **CoreAudio への書き込みは `noErr` を返しても反映されないことがあります。** 往路（別レートへの変更）が効いていなければ I/O コンテキストは張り直されていないので、書き込みの戻り値と読み直した現在値の両方を確認し、失敗はログと終了ステータスに出しています
